      ...
      <div className='mx-auto basis-11/12 lg:basis-10/12'>
        <div className='bg-white rounded-lg p-5 pb-0'>
          <div className='text-center flex flex-wrap justify-between'>
            ...
            <button className='btn-primary text-sm sm:text-base bg-blue-500 hover:bg-blue-700 focus:ring-blue-400 sm:py-0 basis-full sm:basis-3/12 mt-5 sm:mt-0' onClick={nameChk}>시작하기</button>
            <h3 className="my-5 basis-full text-center">{userName !== "" && <> <span className='text-indigo-500 font-bold'>{userName}님, </span><span>문제 유형은 기본값으로 설정되어있으며, 총 {quiz.length}문제 중 한 문제를 선택하셨습니다.</span></>}</h3>
          </div>
        </div>
        ...
        <div className="w-full bg-white rounded-lg p-5 mt-5 flex justify-between flex-wrap items-center">
            ...
            <select className='type border rounded basis-6/12 text-center py-1.5' onChange={ChangeEvent}>
                <option value="전체">전체 ({quiz.length}문제)</option>
                {
                    ...
                    }) // 중괄호를 쓰면 return을 써야하고 안 쓰면 return 생략 가능
                }
            </select>
            </div>
        </div>
